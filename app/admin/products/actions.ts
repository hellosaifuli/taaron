...
export async function createProduct(formData: FormData) {
  ...
  const { data: product, error } = await supabase
    .from('products')
    .insert({ name, description, price, cost, sku, image_url, thumbnail_url, status })
    .select()
    .single()
  ...
  // Insert extra images
  const imageUrls = extra_images.split('\n').map(u => u.trim()).filter(Boolean)
...
export async function updateProduct(id: string, formData: FormData) {
  ...
  const { error } = await supabase
    .from('products')
    .update({ name, description, price, cost, sku, image_url, thumbnail_url, status })
    .eq('id', id)
