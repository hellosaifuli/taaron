...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      siteName: 'Taaron',
      images: [{ url: `${baseUrl}/taaron-logo.png`, width: 1200, height: 630, alt: post.title }],
      type: 'article',
    ...
    <>
      ...
      <div className="min-h-screen bg-[#F7F4EF] text-[#111111]">
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, #2C1F14 0%, #5C3D22 45%, #9B6F47 100%)',
            minHeight: '42vh',
          }}
        >
          <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 30% 70%, #F7F4EF 0%, transparent 55%)' }} />
          <div className="relative mx-auto w-full max-w-screen-lg">
            <FadeInSection from="up">
              ...
              <span className="block text-[9px] uppercase tracking-[0.35em] text-[#C4A882]">{post.category}</span>
              <h1
        ...
        <div className="mx-auto max-w-screen-lg px-6 py-16 lg:grid lg:grid-cols-[1fr_300px] lg:gap-16 lg:px-16 lg:py-20">
          ...
          <article>
            <FadeInSection from="up">
              <p className="mb-10 text-base leading-[1.9] text-[#5C5652]" style={{ fontSize: '1.05rem' }}>
                {post.excerpt}
              </p>
            </FadeInSection>
            ...
            {(post.sections ?? []).map((section, i) => (
              <FadeInSection key={i} delay={i * 40} from="up">
                <div className="mb-8">
                  ...
                  <p className="leading-[1.9] text-[#5C5652]" style={{ fontSize: '0.975rem' }}>
                    {section.body}
                  </p>
                </div>
              </FadeInSection>
            ...
            {/* Tags */}
            <FadeInSection from="up" className="mt-12 border-t border-[#E5DFD6] pt-8">
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-[10px] uppercase tracking-widest text-[#9E9690]">Category</span>
          ...
          <aside className="mt-16 lg:mt-0">
            ...
            <FadeInSection from="right" className="sticky top-28">
              <div className="mb-8 bg-[#111111] p-8">
                <p className="text-[9px] uppercase tracking-widest text-[#9E9690]">Taaron</p>
                <p
                  className="mt-3 text-white"
                  style={{ fontFamily: 'var(--font-display)', fontSize: '1.3rem', fontWeight: 400, lineHeight: '1.3' }}
                >
              ...
              {relatedPosts.length > 0 && (
                <div>
                  <p className="mb-4 text-[10px] uppercase tracking-[0.35em] text-[#9E9690]">More Articles</p>
                  <div className="space-y-px bg-[#E5DFD6]">
                    {relatedPosts.map((p) => (
                      ...
                      >
                        <p className="text-[9px] uppercase tracking-widest text-[#9B6F47]">{p.category}</p>
                        <p
                        ...
                        <p className="mt-1.5 text-xs text-[#9E9690]">{p.readTime}</p>
                      </Link>
                    ))}
                  </div>
                </div>
        ...
        <section className="border-t border-[#E5DFD6] bg-white px-6 py-16 lg:px-16">
          <FadeInSection from="up">
            <div className="mx-auto max-w-screen-lg flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-[0.35em] text-[#9E9690]">Explore the collection</p>
                <p
                  className="mt-2 text-[#111111]"
                  style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.4rem, 3vw, 2rem)', fontWeight: 400 }}
                >
